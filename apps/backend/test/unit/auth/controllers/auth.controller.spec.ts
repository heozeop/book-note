...
describe('AuthController', () => {
  ...
  describe('login', () => {
    it('should login a user and set HTTP-only cookies for both tokens', async () => {
      ...
      // When
      const result = await controller.login(loginDto, 'test-user-agent', '127.0.0.1', mockResponse);
    ...
    it('should throw an error if user is not found after login', async () => {
      ...
      // When & Then
      await expect(controller.login(loginDto, 'test-user-agent', '127.0.0.1', mockResponse))
