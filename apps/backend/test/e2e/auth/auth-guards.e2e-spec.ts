...
describe("Authentication Guards (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      if (authCookies.length === 0) {
        console.error(
          "Auth cookies not received even though login was successful. Check AuthTestUtil.login implementation.",
        );
      } else {
        // Debug cookies
    ...
    it("should enforce request validation", async () => {
      ...
      await request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          const emailError = res.body.message.find(
            (msg: string) =>
  ...
  describe("Auth Guards and Cookie Handling", () => {
    beforeEach(() => {
      if (authCookies.length === 0) {
        console.warn(
          "Auth cookies not available, but continuing test anyway to check for 401 responses",
        );
      } else {
    ...
    it("should handle protected endpoints differently with and without auth cookies", async () => {
      ...
      if (authCookies.length > 0) {
        ...
        expect(responseWithCookies.body.email).toEqual(
    ...
    it("should successfully logout and clear cookies", async () => {
      ...
      const clearingCookies = clearedCookies.filter(
        (cookie) =>
          (cookie.includes("Expires=") && cookie.includes("accessToken=")) ||
