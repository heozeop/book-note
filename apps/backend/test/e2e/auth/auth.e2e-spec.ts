...
describe("Authentication (e2e)", () => {
  ...
  describe("Registration Process", () => {
    ...
    it("should reject registration with weak password", async () => {
      ...
      const hasPasswordRequirement = messages.some(
        (msg: string) => msg.includes("비밀번호") && msg.includes("8자"),
      );
  ...
  describe("Logout Process", () => {
    it("should successfully logout a user and clear cookies", async () => {
      ...
      const hasClearedAccessToken = logoutCookies.some(
        (cookie) =>
          cookie.includes("accessToken=") && cookie.includes("Expires="),
      );

      const hasClearedRefreshToken = logoutCookies.some(
        (cookie) =>
