...
| 영역           | 기술/솔루션                   | 선택 이유                                    |
| ------------- | --------------------------- | ------------------------------------------ |
| ORM          | MikroORM                    | 타입 안전성, 효율적인 UnitOfWork 패턴          |
...
계층형 아키텍처는 관심사를 명확하게 분리하여 유지보수성과 테스트 용이성을 높입니다:

| 계층            | 책임                                      | 구현 방식                           |
| -------------- | ---------------------------------------- | ---------------------------------- |
| 컨트롤러 계층     | 요청 처리, 응답 반환, 검증                  | REST 컨트롤러, GraphQL 리졸버       |
| 서비스 계층      | 비즈니스 로직, 트랜잭션 관리                 | 서비스 클래스, 유틸리티 서비스          |
| 리포지토리 계층   | 데이터 접근, 영속성 로직                    | MikroORM 리포지토리                 |
| 엔티티 계층      | 데이터 모델, 유효성 검증                    | MikroORM 엔티티 클래스              |
| 공통 계층       | 전역 유틸리티, 공통 인터페이스                | 헬퍼, DTO, 예외 클래스               |

#### 9.3.2 모듈 구성

BookNote 애플리케이션은 다음과 같이 기능별 모듈로 구성됩니다:
...
| 모듈             | 책임 영역                                   | 주요 구성요소                         |
| --------------- | ------------------------------------------ | ---------------------------------- |
| 인증 모듈         | 사용자 등록, 인증, 권한 관리                  | 컨트롤러, 서비스, 리포지토리, JWT 전략   |
| 서재 모듈         | 책 등록, 컬렉션 관리, 메타데이터              | 컨트롤러, 서비스, 리포지토리, 엔티티     |
| 독서 노트 모듈     | 페이지 노트, 구절, 생각 트리                  | 컨트롤러, 서비스, 리포지토리, 엔티티     |
| 독서 상태 모듈     | 읽기 상태 관리, 진행 상황 추적                | 컨트롤러, 서비스, 리포지토리, 엔티티     |
| 통계 모듈         | 독서 분석, 통계 데이터                        | 컨트롤러, 쿼리 서비스, 데이터 집계      |
| 사용자 설정 모듈    | 환경 설정, 개인화                           | 컨트롤러, 서비스, 리포지토리, 엔티티     |
| 소셜 공유 모듈     | 노트 공유, 리액션, 권한 관리                   | 컨트롤러, 서비스, 접근 제어 로직        |
...
#### 9.4.1 백엔드 폴더 구조 (NestJS + 계층형)

```
src/
├── main.ts                          # 애플리케이션 엔트리 포인트
├── app.module.ts                    # 루트 모듈
├── common/                          # 공유 유틸리티, 기본 클래스
│   ├── dtos/                        # 공통 DTO
...
| 비즈니스 로직        | - 서비스 계층 최적화<br>- 읽기 전용 쿼리<br>- 효율적인 알고리즘 | 비즈니스 로직 처리 속도 35% 향상        |
| 메모리 관리          | - 커넥션 풀링<br>- 객체 재사용<br>- 가비지 컬렉션 조정 | 메모리 사용 25% 감소                  |
